...
// Banner
var banner = `/*! ${configs.name ? configs.name : pkg.name} v${pkg.version} | (c) ${new Date().getFullYear()} ${pkg.author.name} | ${pkg.license} License | ${pkg.repository.url} | Open Source: ${pkg.openSourceCredits} */`;
...
var createOutput = function (filename, minify) {
	return configs.formats.map(function (format) {
		...
		if (format === 'iife') {
			output.name = configs.name ? configs.name : pkg.name.replace(/\-/g, '');
		}
		if (minify) {
